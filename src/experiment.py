...
class Sparse_Experiment(Experiment):
    ...
    #def __init__(self, model, batch_size, mapping, experiment_id='experiment', l1=0):
    ...
    def compute_loss(self, x, beta, gamma, loss_type='se', training=False, IWAE=1):
        ...
        if IWAE>0:
            ...
            KL=logqz_x-logpz
        #if self.l1>0:
        #    loss = -tf.reduce_mean(elbo) + sparse_loss*gamma + tf.reduce_mean(tf.abs(z))*self.l1
        #else:
    ...
    def compute_apply_gradients(self, x, beta, gamma, loss_type='se', IWAE=0):
        ...
        self.KL_term_record(KL)
        self.reconstruction_logprob_record(reconstruction)  
        #!return loss_info[0], KL, reconstruction #!
    ...
    def train_epoch(self, train_dataset, beta, gamma, loss_type='se', IWAE=0):
        ...
        for train_x in train_dataset:
            tem=self.compute_apply_gradients(train_x, beta, gamma, loss_type=loss_type, IWAE=IWAE) #!
            #!print(tem) 
        end_time = time()
    ...
    def test_epoch(self, test_dataset, beta, gamma, loss_type='se', IWAE=0):
        ...
        self.sparse_loss_record.reset_states()
        for test_x in test_dataset:
            loss, elbo, KL, reconstruction , info = self.compute_loss(test_x, beta, gamma, loss_type=loss_type, IWAE=IWAE)
            self.loss_record(loss)
            self.ELBO_record(elbo)
            self.KL_term_record(KL)
            self.reconstruction_logprob_record(reconstruction)
...
class Vamp_Experiment(Sparse_Experiment):
    ...
    @tf.function
    def compute_apply_gradients(self, x, beta, gamma, loss_type='se', **kwargs):
        with tf.GradientTape() as tape:
...
class MoG_Experiment(Sparse_Experiment):
    ...
    @tf.function
    def compute_loss(self, x, beta, gamma, loss_type='se', training=False, **kwargs):
        mean, logvar = self.model.encode(x, training=training)
        z = reparameterize(mean, logvar)
        if self.mapping:
            z, info=self.mapping(z)
