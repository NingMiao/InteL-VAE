...
import src.experiment_flow as Experiment_flow
...
parser.add_argument("--multiplier1", type=int, default=0)
parser.add_argument("--multiplier2", type=int, default=0)
...
parser.add_argument("--restore_best", action='store_true', default=False)  ##Careful
...
#L1 loss
parser.add_argument("--l1", type=float, default=0.0) #! Baseline: force sparsity by l1 loss
...
if args.flow:
    experiment_id+='_flow_'+str(args.MoG_num)
...
if args.dataset=='mnist' and args.cut_by_labels_dim!=0: #Only load sample with certain label
    ...
    if args.multiplier1>0:
        multiplier=[args.multiplier1, args.multiplier2]
    else:
        multiplier=[]
...
train_dataset, test_dataset, test_dataset_for_logP, image_shape=load_dataset(args.dataset, args.batch_size, args.logP_batch_size, labels_for_mnist=labels_for_mnist, test_dataset_seed=args.test_dataset_seed, multiplier=multiplier)
...
if args.mapping=='sparse':
    ...
    with tf.device('/device:GPU:0'):
        cluster=Cluster(args.cluster_num)
...
if args.mapping=='':
    ...
    if args.vamp:
        ...
        experiment=Experiment_flow.Flow_Experiment(model, args.batch_size, None, experiment_id=experiment_id)
    else:
        #!experiment=Experiment.Sparse_Experiment(model, args.batch_size, None, experiment_id=experiment_id, l1=args.l1)
...
if args.test_only:
    ...
    for seed in range(1):
        ...
        _, test_dataset, test_dataset_for_logP, _=load_dataset(args.dataset, args.batch_size, args.logP_batch_size, labels_for_mnist=labels_for_mnist, test_dataset_seed=seed)
...
for epoch in range(initial_epoch+1, initial_epoch+args.epoch + 1):
    ...
    if args.multiplier1>0:
        br=cluster.l.get_weights()[1]
        br_exp=np.exp(br)
        #boundary=4*(br_exp/(1+br_exp))-2
        boundary=8*(br_exp/(1+br_exp))-4
        prob=norm.cdf(boundary)
        String+='\n Learned splitting: {}'.format(prob)
        if epoch==initial_epoch+args.epoch:
            with open('log/uneven_log.txt', 'a') as g:
                pass
                #!g.write(str(args.multiplier1)+'\t'+str(args.multiplier2)+'\t'+str(prob[0])+'\n')#! For explore uneven dataset only
    ...
    if epoch % args.test_every ==0: 
        ...
        if args.FID and args.leading_metric=='FID':
            stall_flag=(FID_list[-1]>=np.min(FID_list[:-1]))
        elif args.logP and args.leading_metric=='logP':
            stall_flag=(logP_list[-1]<=np.max(logP_list[:-1]))
        else:
